...
ssh $AMAZON_USER@$OP_IP -i $AMAZON_PEM_FILE << EOF
    ...
    docker create -ti --name dummy ${AMAZON_USER}-op bash
    rm -Rf op_certs
...
ssh $AMAZON_USER@$RP_IP -i $AMAZON_PEM_FILE << EOF
    ...
    docker create -ti --name dummy ${AMAZON_USER}-rp bash
    rm -Rf rp_certs
...
#on OP
ssh $AMAZON_USER@$OP_IP -i $AMAZON_PEM_FILE << EOF
    OP_IP=$OP_IP RP_IP=$RP_IP TLS_SIGN=rsa JWT_SIGN=rsa LOG_LEVEL=DEBUG docker compose -f docker-compose-amazon.yml up -d op op_clone
EOF
